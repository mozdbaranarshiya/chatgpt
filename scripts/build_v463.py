...
DISCOVERY_JS = ROOT / "patches" / "v4.6.3" / "commerce-discovery.js"
...
def main() -> None:
    ...
    if not DISCOVERY_JS.exists():
        raise FileNotFoundError(DISCOVERY_JS)
    ...
    with tempfile.TemporaryDirectory(prefix="gptyar-v463-") as tmp_name:
        ...
        shutil.copy2(DISCOVERY_JS, extension_root / "commerce-discovery.js")
        ...
        script_tags = (
            '  <script src="commerce-admin.js"></script>\n'
            '  <script src="commerce-discovery.js"></script>\n'
        )

        if "commerce-admin.js" not in html or "commerce-discovery.js" not in html:
            ...
            html = html.replace(
                anchor,
                anchor
                + '  <!-- Commerce admin: authenticated coupon/catalog management -->\n'
                + script_tags,
                1,
            )
        ...
        with zipfile.ZipFile(
            ...
            for path in sorted(extension_root.rglob("*")):
                if path.is_file():
                    target.write(
                        path,
                        Path("v4.6.3") / path.relative_to(extension_root),
                    )
